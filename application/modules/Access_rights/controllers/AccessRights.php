...
class AccessRights extends MX_Controller {

    public function __construct(){
        parent::__construct();
        $this->load->model('AccessRight');
    ...
    {
        $html='';
        $roleLists = $this->AccessRight->get_role_list();
    ...
    public function menu() {
        ...
        $menu = '';
        $parentMenus = $this->AccessRight->get_menu_parent($UAId);
        ...
        foreach ($parentMenus as $parentMenu) {
            ...
            if ($this->AccessRight->hasChild($UAId, $parentMenu->MenuId)) {
                $childMenus = $this->AccessRight->get_menu_child($UAId, $parentMenu->MenuId);
    ...
    public function append_menu_child($UAId, $childMenus, $parentId) {
        ...
        foreach ($childMenus as $childMenu) {
            ...
            if ($this->AccessRight->hasChild($UAId, $childMenu->MenuId)) {
                $menu .='<tr>
                            <td>
                ...
                $childMenuslvl3 = $this->AccessRight->get_menu_child($UAId, $childMenu->MenuId);
    ...
    public function access_rights_icon($status, $id, $interface, $UAId, $isParent) {
        ...
        if ($isParent && $interface != '_view') {
            ...
            $res = $status == 1 ? '<i data-rid="'. $id .'" data-menu-id="'. $id .'" data-infa="'. $interface .'" class="material-icons fa-check has-rights fa-trig">security</i> ' : '<i data-rid="'. $UAId .'" data-menu-id="'. $id .'" data-infa="'. $interface .'" class="material-icons fa-times no-rights fa-trig">security</i>';
        }
        
        return $res .= '<div class="preloader pl-size-xs hideBtn">
    ...
    public function stat_update() {
        ...
        $res = $this->AccessRight->stat_update($id, $stat, $interface, $rid);
    ...
    public function getRoleAccessRights() {
        ...
        $menu = '';
        $parentMenus = $this->AccessRight->role_get_menu_parent($RoleId, 0, 0);
        ...
        foreach ($parentMenus as $parentMenu) {
            ...
            if ($this->AccessRight->hasChild('', $parentMenu->MenuId)) {
                $childMenus = $this->AccessRight->role_get_menu_child($parentMenu->MenuId, $RoleId);
    ...
    public function role_append_menu_child($RoleId, $childMenus, $parentId) {
        ...
        foreach ($childMenus as $childMenu) {
            ...
            if ($this->AccessRight->hasChild('', $childMenu->MenuId)) {
                $menu .='<tr>
                            <td>
                                <div class="tt" data-tt-id="'. $childMenu->MenuId .'" data-tt-parent="'.$parentId.'"><span class="margin-5 material-icons">'. $childMenu->MenuIcon .'</span>'. $childMenu->MenuName .'</div>
                            </td>
                ...
                $parentId2 = $childMenu->MenuId;
                $childMenuslvl3 = $this->AccessRight->role_get_menu_child($parentId2, $RoleId);
                // echo "<pre>";
    ...
    public function ar_update(){
        ...
        $res = $this->AccessRight->ar_update($stat, $interface, $rid, $menuId);
    ...
    public function employee_list(){
            ...
            $table = 'user_access';
            $column_order = array('fname','lname','UAId'); 
            $column_search = array('fname','lname','UAId'); 
            $order = array('UAId' => 'asc'); 

            $dataTables = modules::load('Datatables/Datatables/');
            $EmpLists = $dataTables->get_data_view($table, $column_order, $column_search, $order);
            ...
            foreach ($EmpLists as $EmpList) {
                ...
                $row = array();
                $row[] = $EmpList->UAId;
                $row[] = ucwords($EmpList->lname);
                $row[] = ucwords($EmpList->fname);
                $row[] = '<button type="button" id="'. $EmpList->UAId .'" class="btn btn-primary btn-xs waves-effect accessRightOpen">
                                    <i class="material-icons">search</i>
                                </button><div class="preloader pl-size-xs hideBtn">
                                    <div class="spinner-layer pl-blue">
                                        <div class="circle-clipper left">
                                            <div class="circle"></div>
                                        </div>
